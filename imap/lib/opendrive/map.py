    ...
    def post_process(self):
        ...
        for junction_id, junction in self.junctions.items():
            for connection in junction.connections:
                # Skip invalid connections with -1 or missing road IDs
                if connection.incoming_road and connection.incoming_road != '-1' and connection.incoming_road in self.roads:
                    connection.incoming_road_obj = self.roads[connection.incoming_road]
                else:
                    connection.incoming_road_obj = None
                
                if connection.connecting_road and connection.connecting_road != '-1' and connection.connecting_road in self.roads:
                    connection.connecting_road_obj = self.roads[connection.connecting_road]
                else:
                    connection.connecting_road_obj = None
