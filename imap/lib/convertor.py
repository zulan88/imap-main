...
class Opendrive2Apollo(Convertor):
    ...
    def _get_coordinate_transformer(self):
        """延迟创建并缓存投影转换器，避免重复初始化带来的性能损耗。"""
        if self._transformer is None:
            if not self.ori_proj_txt:
                raise ValueError("ori_proj_txt 未设置，无法进行坐标转换")
            crs_tmerc = CRS.from_proj4(self.ori_proj_txt)
    ...
    def apply_coordinate_transform(self, x_tmerc, y_tmerc):
        """应用坐标转换，将x,y坐标转换为目标坐标系"""
        transformer = self._get_coordinate_transformer()

        # 执行转换（支持单个坐标或批量坐标）
        if isinstance(x_tmerc, (int, float)) and isinstance(y_tmerc, (int, float)):
            # 单个坐标
            x_utm, y_utm = transformer.transform(x_tmerc, y_tmerc)
        else:
            # 批量坐标（如列表、数组）
            x_utm, y_utm = transformer.transform(x_tmerc, y_tmerc)
        return x_utm, y_utm
    ...
    def convert_proj_txt(self, proj_txt):
        print("ori_proj_txt: {}".format(proj_txt))
        self.ori_proj_txt = proj_txt
        # 投影配置变更后需要重新构建转换器
        self._transformer = None
        if proj_txt is None:
    ...
    def add_lane_boundary(self, pb_lane, lane):
        ...
        for point3d in lane.left_boundary:
            ...
            # 应用坐标转换
            transformed_x, transformed_y = self.apply_coordinate_transform(point3d.x, point3d.y)
            # lhd 2022/12/03 for 3D view
            if global_var.get_element_value("enable_z_axis"):
                point.x, point.y, point.z = transformed_x, transformed_y, point3d.z
            else:
                point.x, point.y = transformed_x, transformed_y
        segment.s = 0
        # 应用坐标转换到起始位置
        transformed_x, transformed_y = self.apply_coordinate_transform(
            lane.left_boundary[0].x, lane.left_boundary[0].y)
        segment.start_position.x = transformed_x
        segment.start_position.y = transformed_y
        ...
        # 应用坐标转换到起始位置
        transformed_x, transformed_y = self.apply_coordinate_transform(
            lane.center_line[0].x, lane.center_line[0].y)
        segment.start_position.x = transformed_x
        segment.start_position.y = transformed_y
        ...
        # 应用坐标转换到起始位置
        transformed_x, transformed_y = self.apply_coordinate_transform(
            lane.right_boundary[0].x, lane.right_boundary[0].y)
        segment.start_position.x = transformed_x
        segment.start_position.y = transformed_y
    ...
    def add_road_section_curve(self, pb_boundary_edge, boundary, length):
        ...
        segment.s = 0
        # 应用坐标转换到起始位置
        transformed_x, transformed_y = self.apply_coordinate_transform(boundary[0].x, boundary[0].y)
        segment.start_position.x = transformed_x
        segment.start_position.y = transformed_y
    ...
    def convert_junctions(self):
        for _, xodr_junction in self.xodr_map.junctions.items():
            ...
            for x, y in polygon:
                ...
                # 应用坐标转换
                transformed_x, transformed_y = self.apply_coordinate_transform(x, y)
                pb_point.x, pb_point.y, pb_point.z = transformed_x, transformed_y, 0
